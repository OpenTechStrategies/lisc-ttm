...
if ($USER->has_site_access($Enlace_id)) {
    //make the program access strings
    $program_array = $USER->program_access($Enlace_id);
    $enlace_session_string = " AND  ( Session_Names.Program_ID = " . $program_array[0];
    $enlace_program_string = "AND  ( Session_Names.Program_ID = " . $program_array[0];
    foreach ($program_array as $program){
        $enlace_session_string .= " OR Session_Names.Program_ID = " . $program;
        $enlace_program_string .= " OR Programs.Program_ID = " . $program;
    }
    $enlace_session_string .= ")";
    $enlace_program_string .= ")";
}
else{
    $enlace_session_string = "";
    $enlace_program_string = "";
...
function generalized_download($download_name, $permissions, $enlace_session_string, $enlace_program_string){
    $download_list_array=array(
        ...
        'SELECT Participants.Participant_ID, First_Name, Last_Name, Participants.Address_Num, Participants.Address_Dir, Participants.Address_Street, Participants.Address_Street_Type, Address_City, Address_State, Address_ZIP, DOB, Age, Gender, Grade, Institution_Name, Roles.Role FROM Participants LEFT JOIN Roles ON Participants.Role = Roles.Role_ID LEFT JOIN Institutions ON School = Inst_ID', 
        'non_admin_string' => ' LEFT JOIN Participants_Programs ON Participants.Participant_ID = Participants_Programs.Participant_ID INNER JOIN Session_Names ON Participants_Programs.Program_ID = Session_Names.Session_ID INNER JOIN Programs ON Session_Names.Program_ID = Programs.Program_ID WHERE Programs.Program_ID IS NOT NULL ' . $enlace_program_string,
        'add_access' => '1',
        'titles' => array("Participant ID", "First_Name", "Last_Name", "Address Number", "Address Direction", "Address Street Name", "Address Street Type", "Address_City", "Address_State", "Address_ZIP", "DOB", "Age", "Gender", "Grade", "School", "Role")), 
        ...
        'SELECT Assessment_ID, Assessments.Participant_ID, Participants_Caring_Adults.Program, Session_Names.Session_Name, Programs.Name, DOB, Age, Grade, Institution_Name, Home_Language, Ethnicity, Race, Recruitment, Youth_Behavioral, Assessments.Pre_Post, Assessments.Date_Logged, DATE(Participants_Caring_Adults.Date_Logged), BYS_1, BYS_2, BYS_3, BYS_4, BYS_5, BYS_6, BYS_7, BYS_8, BYS_9, BYS_E, BYS_T, JVQ_1, JVQ_2, JVQ_3, JVQ_4, JVQ_5, JVQ_6, JVQ_7, JVQ_8, JVQ_9, JVQ_E, JVQ_T, JVQ_12, Check_In, Know_You, Compliment, Crisis_Help, Pay_Attention, KnowImportance, Personal_Advice, Upset_Discussion, Friends, Finish_HS, Stay_Safe, Alive_Well, Happy_Life, Manage_Work, Proud_Parents, Solve_Problems, Interesting_Life, Coping, Cowardice, Self_Care, Anger_Mgmt, Negotiation, Self_Defense, Handle_Others, Self_Awareness, Parent_Approval, Parent_Disapproval, Teasing_Prevention FROM Assessments LEFT JOIN Participants_Baseline_Assessments ON Baseline_Assessment_ID = Baseline_ID LEFT JOIN Participants_Caring_Adults ON Caring_ID = Caring_Adults_ID LEFT JOIN Participants_Future_Expectations ON Future_Id = Future_Expectations_ID LEFT JOIN Participants_Interpersonal_Violence ON Violence_ID = Interpersonal_Violence_ID LEFT JOIN Participants ON Assessments.Participant_Id = Participants.Participant_ID LEFT JOIN Participants_Programs ON Participants.Participant_ID = Participants_Programs.Participant_ID INNER JOIN Session_Names ON Participants_Programs.Program_ID = Session_Names.Session_ID INNER JOIN Programs ON Session_Names.Program_ID = Programs.Program_ID ',
        'non_admin_string' => ' ',
        'non_admin_string2' => $enlace_session_string,
        'add_access' => 1,
        'query2' => ' WHERE Assessments.Pre_Post = 1 ',
        'titles' => array("Assessment ID", "Participant ID", "Program ID", "Program Name",  "Program Site/Org", "DOB", "Age", "Grade", "School", "Home Language other than English", "Ethnicity", "Race", "Recruitment Method", "Youth Behavioral History", "Pre or Post", "Date Logged", "Date Administered"),
        ...
        'SELECT Assessment_ID, Assessments.Participant_ID, Participants.First_Name, Participants.Last_Name, Participants_Caring_Adults.Program, Session_Names.Session_Name, Programs.Name, Assessments.Pre_Post, Assessments.Date_Logged, DATE(Participants_Caring_Adults.Date_Logged), Check_In, Know_You, Compliment, Crisis_Help, Pay_Attention, KnowImportance, Personal_Advice, Upset_Discussion, Friends, Finish_HS, Stay_Safe, Alive_Well, Happy_Life, Manage_Work, Proud_Parents, Solve_Problems, Interesting_Life, Coping, Cowardice, Self_Care, Anger_Mgmt, Negotiation, Self_Defense, Handle_Others, Self_Awareness, Parent_Approval, Parent_Disapproval, Teasing_Prevention FROM Assessments LEFT JOIN Participants_Caring_Adults ON Caring_ID = Caring_Adults_ID LEFT JOIN Participants_Future_Expectations ON Future_Id = Future_Expectations_ID LEFT JOIN Participants_Interpersonal_Violence ON Violence_ID = Interpersonal_Violence_ID LEFT JOIN Participants ON Assessments.Participant_Id = Participants.Participant_ID LEFT JOIN Participants_Programs ON Participants.Participant_ID = Participants_Programs.Participant_ID INNER JOIN Session_Names ON Participants_Programs.Program_ID = Session_Names.Session_ID INNER JOIN Programs ON Session_Names.Program_ID = Programs.Program_ID ',
        'non_admin_string' => ' ',
        'non_admin_string2' => $enlace_session_string,
        'add_access' => 1,
        ...
        'SELECT Assessment_ID, Assessments.Participant_ID, Participants_Caring_Adults.Program, Session_Names.Session_Name, Programs.Name, Assessments.Pre_Post, Assessments.Date_Logged, DATE(Participants_Caring_Adults.Date_Logged), Check_In, Know_You, Compliment, Crisis_Help, Pay_Attention, KnowImportance, Personal_Advice, Upset_Discussion, Friends, Finish_HS, Stay_Safe, Alive_Well, Happy_Life, Manage_Work, Proud_Parents, Solve_Problems, Interesting_Life, Coping, Cowardice, Self_Care, Anger_Mgmt, Negotiation, Self_Defense, Handle_Others, Self_Awareness, Parent_Approval, Parent_Disapproval, Teasing_Prevention FROM Assessments LEFT JOIN Participants_Caring_Adults ON Caring_ID = Caring_Adults_ID LEFT JOIN Participants_Future_Expectations ON Future_Id = Future_Expectations_ID LEFT JOIN Participants_Interpersonal_Violence ON Violence_ID = Interpersonal_Violence_ID LEFT JOIN Participants ON Assessments.Participant_Id = Participants.Participant_ID LEFT JOIN Participants_Programs ON Participants.Participant_ID = Participants_Programs.Participant_ID INNER JOIN Session_Names ON Participants_Programs.Program_ID = Session_Names.Session_ID INNER JOIN Programs ON Session_Names.Program_ID = Programs.Program_ID ',
        'non_admin_string' => ' ',
        'non_admin_string2' => $enlace_session_string,
        'add_access' => 1,
        'query2' => ' WHERE Assessments.Pre_Post = 2 ',
        'titles' => array("Assessment ID", "Participant ID", "Session ID (of program)", "Session Name",  "Program Name", "Pre or Post", "Date Logged", "Date Administered"),
        'legend' => array("(id)", "(id)", "(id)", "session", "(program)", "(pre or post)", "(date)", "(date)")),
...
'titles' => array("Referral ID", "ID of referred person", "First name of referred person", "Last name of referred person", "Referrer First Name", "Referrer Last Name", "Referring Institution", "Referring Program", "Program referred to", "Date Logged")),
        ...
        'enlace_total_dosage' => array('db' => 'enlace', 'query' => 'SELECT Participants.Participant_ID, First_Name, Last_Name FROM Participants', 
        ...
        'enlace_total_dosage_deid' => array('db' => 'enlace', 'query' => 'SELECT Participants.Participant_ID FROM Participants', 
        'non_admin_string' => ' LEFT JOIN Participants_Programs on Participants.Participant_ID = Participants_Programs.Participant_ID INNER JOIN Session_Names on Participants_Programs.Program_ID = Session_Names.Session_ID ',
        'non_admin_string2' => $enlace_session_string,
        'query2' => ' WHERE Participants.Participant_ID IS NOT NULL ',
        'add_access' => 1,
        ...
        'titles' => array("Participant ID", "First Name", "Last Name", "Program ID", "Program Name",  "Program Site/Org", "Date Entered, Pre Survey", "Date Administered, Pre Survey", "Date Entered, Post Survey", "Date Administered, Post Survey", "DOB", "Age", "Grade", "School", "Home Language", "Ethnicity", "Race", "Recruitment Method", "Youth Behavioral History"),
...
LEFT JOIN
        ...
        'SELECT Participants.First_Name, Participants.Last_Name, LC_Basics.* FROM LC_Basics LEFT JOIN Participants ON LC_Basics.Participant_ID = Participants.Participant_ID',
        ...
        'SELECT * FROM LC_Basics LEFT JOIN Participants ON LC_Basics.Participant_ID = Participants.Participant_ID',
        'titles' => array("Unique ID", "Participant ID", "Household Size", "Parent1 AGI", "Parent2 AGI", "Student AGI", "ACT Score", "High School GPA", "Dependency Status", "Father's Highest Level of Education", "Mother's Highest Level of Education", "Student Aspiration", "First Generation College Student", "Persistence/Graduation", "Student Hometown", "Student High School", "Number of Scholarship Applications", "Number of Scholarships Received", "Scholarship Volume Offered", "Scholarships Volume Received", "Household Income", "AMI", "Move In Date", "Move Out Date", "24 or older?", "Masters Degree?", "Married?", "Has Children?", "Homeless?", "Self Sustaining?")),

        'trp_lc_by_term' => array('db' => 'TRP', 'query' =>
        'SELECT First_Name, Last_Name, LC_Terms.*, College_Name FROM LC_Terms LEFT JOIN Participants ON LC_Terms.Participant_ID = Participants.Participant_ID LEFT JOIN Colleges ON LC_Terms.College_ID = Colleges.College_ID',
        'titles' => array("First Name", "Last Name", "Term_ID", "Participant_ID", "College_ID", "Term_Type", "Term", "School_Year", "Credits", "Subsidized_Loan", "Unsubsidized_Loan", "Major", "Minor", "Expected_Match", "Actual_Match", "College_GPA", "Internship_Status", "Intern_Hours", "Dropped_Classes", "Dropped_Credits", "College_Name")),

        'trp_lc_by_term_deid' => array('db' => 'TRP', 'query' =>
        'SELECT LC_Terms.*, College_Name FROM LC_Terms  LEFT JOIN Colleges ON LC_Terms.College_ID = Colleges.College_ID',
        'titles' => array("Term_ID", "Participant_ID", "College_ID", "Term_Type", "Term", "School_Year", "Credits", "Subsidized_Loan", "Unsubsidized_Loan", "Major", "Minor", "Expected_Match", "Actual_Match", "College_GPA", "Internship_Status", "Intern_Hours", "Dropped_Classes", "Dropped_Credits", "College_Name")),

        'trp_lc_colleges' => array('db' => 'TRP', 'query' =>
        'SELECT * FROM Colleges',
        'titles' => array("College ID", "College Name", "College Type (2-year or 4-year)", "Selectivity")),

        'trp_lc_xls_emulator' => array('db' => 'TRP', 'query' =>
        'SELECT LC_Basics.Participant_ID, Cohort, Status, Handbook, Floor, Pod, Room_Number, Key_Card_Number, Transcript_Submitted, Service_Hours_Submitted, Participants.First_Name, Participants.Last_Name, DOB,  LCRC_Username, LCRC_Password, LCRC_Print_Code, Gender, Roommate, Application_Received, Application_Completed, Email, Email_2, Address_Street_Num, Address_Street_Direction, Address_Street_Name, Address_Street_Type, Address_City, Address_State, Address_Zipcode, Participants.Phone, Mobile_Phone, Student_High_School, ACT_Score, High_School_GPA, HS_GPA_Weighted, Expected_Graduation_Year, College_Grade_Level, Major, Minor, College_Name, College_Type, Selectivity, Expected_Match, Actual_Match, Credits, College_GPA, School_Year, Term_Type, Dropped_Classes, Dropped_Credits,  Reason_Leave, Reason_Stay, Student_Aspiration, Mother_Highest_Level_Education, Father_Highest_Level_Education, First_Generation_College_Student, Internship_Status, Intern_Hours,  Mid_Twenties, Masters_Degree, Married, Military, Has_Children, Homeless, Self_Sustaining, Dependency_Status, Tax_Exemptions, Household_Size, Household_Size_TRP, Parent1_AGI, Parent2_AGI, Student_AGI, AMI,  Tuition, Mandatory_Fees, College_Cost, Pell_Grant, MAP_Grant, University_Scholarship, Subsidized_Loan, Unsubsidized_Loan, Savings, Family_Help, LC_Scholarship, Application_Source, Notes, Email_Pack, Email_Orientation, Email_Roommate, Move_In_Date, Move_In_Time, Move_In_Registration, Move_In_Address, Move_In_Note, Orientation_Date, Orientation_Time,  Emergency_Contacts.First_Name, Emergency_Contacts.Last_Name, Emergency_Contacts.Phone, Emergency_Contacts.Relationship  FROM LC_Basics LEFT JOIN Participants on Participants.Participant_ID = LC_Basics.Participant_ID LEFT JOIN LC_Terms ON LC_Basics.Participant_ID = LC_Terms.Participant_ID LEFT JOIN Emergency_Contacts ON LC_Basics.Participant_ID = Emergency_Contacts.Participant_ID LEFT JOIN Colleges on LC_Terms.College_ID = Colleges.College_ID GROUP BY LC_Basics.Participant_ID ',
        'titles' => array("Database ID", "Group", "Status", "Hndbk", "Floor", "Pod", "Room #", "Key Card #", "Transcript Submitted (Fall)", "20 Service Hours Submitted", "First", "Last", "Date of Birth", "LCRC Username", "LCRC Password", "LCRC Print Code", "Gender", "Roommate", "Application Received (date)", "Application Completed (date)", "Email", "e-Mail 2", "Mailing Address", "", "", "", "Mailing City", "Mailing State", "Zip Code", "Home Phone", "Mobile", "High School", "ACT Score", "HS GPA (Unweighted)", "HS GPA (Weighted)", "Expected Graduation Date", "College Gade Level", "Major", "Minor", "College Name", "College Type", "Selectivity      (of Current College)", "Student's Expected Match Level", "Student's Actual Match Level", "Credits acrrued", "GPA", "Term Year", "Term Season", "Dropped classes", "If dropped classes,  how many credits?",  "Why did you leave La Casa?", "Why did you stay at La Casa?", "Highest Level of Ed (GOAL)", "Highest level of Ed completed by Mother", "Highest level of Ed completed by Father", "First Generation", "Internship for 2014-2105? -- fall 2014", "If so,  where and how many hours per term?", "Age 24 or older? (No=0  | Yes=1)", "Master or Above? (No=0  | Yes=1)", "Married?  (No=0  | Yes=1)", "Military?     (No=0  | Yes=1)", "Has children? (No=0  | Yes=1)", "Homeless?  (No=0  | Yes=1)", "Self Sutaining (If Stdt income = $12, 510)                    (No=0  | Yes=1)", "Dependency Status", "Tax Exemptions", "Household size (per La Casa Application)", "Household Size for TRP", "Parent 1 AGI", "Parent2 AGI", "Student's AGI",  "Actual AMI", "Tuition", "Mandatory Fees (Do not include Loan fees)", "Actual COST OF ATTENDENCE (According to College)", "Pell Grant", "MAP Grant", "University Scholarships", "Federal Direct Subsidized Loan (enter $3, 500 for 4-year or $0 for CC)", "Federal Direct Unsubsidized Loan               (Enter $0 for all)", "Savings", "Family Assistance/Sponsor/Other", "La Casa Scholarship (Annual Award)", "Application Source", "Notes", "e-Mail: Pack This!", "e-Mail: Move-in & Orientation", "e-Mail: Roommate", "Move-In", "Move in time", "Move-In Registrtion", "Move in Address", "Move-in Note", "Orientation Date", "Orientation Time", "Emergency Contact  (First Name)", "Emergency Contact (Last Name)", "Emergency Contact (Phone)", "Emergency Contact (Relationship)"))
...
generalized_download($_GET['download_name'], $USER->site_permissions, $enlace_session_string, $enlace_program_string);
//generalized_download('enlace_session_attendance', $USER->site_permissions, $enlace_session_string, $enlace_program_string);
?>