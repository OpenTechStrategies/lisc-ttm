...
class GoogleMapAPI {
    ...
    function getHeaderJS() { 
        ...
        $scriptUrl = "http://maps.google.com/maps/api/js?key=AIzaSyBPbDfunlbUrCbg1fRmQ0dUCi_4WuGIWrk&sensor=".(($this->mobile==true)?"true":"false");
	...
	*/
    ...
    function geoGetCoords($address,$depth=0) {
        switch($this->lookup_service) {
            case 'GOOGLE':
                $_url = sprintf('http://%s/maps/api/geocode/json?key=AIzaSyBPbDfunlbUrCbg1fRmQ0dUCi_4WuGIWrk&sensor=%s&address=%s',$this->lookup_server['GOOGLE'], $this->mobile==true?"true":"false", rawurlencode($address));
                $_result = false;
