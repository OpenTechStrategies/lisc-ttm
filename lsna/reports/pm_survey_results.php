...
<!--<script language="javascript" type="text/javascript" src="/include/jquery.jqplot.1.0.4r1121/jquery.min.js"></script>-->
    ...
    <form action="reports.php" method="post">
        ...
        <select id="year" name="year">
            <option value="">-----</option>
            <option value="2011" <?php echo($_POST['year'] == '2011' ? 'selected="selected"' : null ) ?>>2011-2012</option>
            <option value="2012" <?php echo($_POST['year'] == '2012' ? 'selected="selected"' : null ) ?>>2012-2013</option>
            <option value="2013" <?php echo($_POST['year'] == '2013' ? 'selected="selected"' : null ) ?>>2013-2014</option>
            <option value="2014" <?php echo($_POST['year'] == '2014' ? 'selected="selected"' : null ) ?>>2014-2015</option>
            <option value="2015" <?php echo($_POST['year'] == '2015' ? 'selected="selected"' : null ) ?>>2015-2016</option>
            <option value="2016" <?php echo($_POST['year'] == '2016' ? 'selected="selected"' : null ) ?>>2016-2017</option>
        </select>

        <select id="school" name="school_post">
            <option value="">----------</option>
            <?php
    ...
    AVG(Self_Efficacy_S) FROM Parent_Mentor_Survey WHERE Pre_Post=1 " . $year_post . $school_post;
//echo $get_pre_averages;
    ...
    include "../include/dbconnopen.php";
    $get_mid_averages = "SELECT AVG(Student_Involvement_B), AVG(Student_Involvement_H),
    AVG(School_Network_I), AVG(School_Network_J), AVG(School_Involvement_M),
    AVG(School_Involvement_P), AVG(School_Involvement_Q),
    AVG(Self_Efficacy_S) FROM Parent_Mentor_Survey WHERE Pre_Post=2 " . $year_post . $school_post;
    ...
    include "../include/dbconnopen.php";
    $get_post_averages = "SELECT AVG(Student_Involvement_B), AVG(Student_Involvement_H),
    AVG(School_Network_I), AVG(School_Network_J), AVG(School_Involvement_M),
    AVG(School_Involvement_P), AVG(School_Involvement_Q),
    AVG(Self_Efficacy_S) FROM Parent_Mentor_Survey WHERE Pre_Post=3 " . $year_post . $school_post;
    ...
    $count_posts = "SELECT * FROM Parent_Mentor_Survey WHERE Pre_Post='2'" . $year_post . $school_post;
    include "../include/dbconnopen.php";
    $posts = mysqli_query($cnnLSNA, $count_posts);
    $num_posts = mysqli_num_rows($posts);
    include "../include/dbconnclose.php";

    $count_laters = "SELECT * FROM Parent_Mentor_Survey WHERE Pre_Post='3'" . $year_post . $school_post;
    include "../include/dbconnopen.php";
    ...
    <table style="font-size: .8em;
        ...
        $text_array = array('B.	Talk with your child’s teacher?', 'H.	Read with your child at home?',
            ...
            'U. In general, I think that I can obtain outcomes that are important to me.');
            ...
            ?><tr>
                ...
                for ($i = 1; $i < 4; $i++) {
                    ...
                    if ($question=='School_Network_I' || $question==School_Network_J || $question==School_Involvement_M){
                        $call_for_arrays="SELECT $question, COUNT(*) AS count FROM Parent_Mentor_Survey GROUP BY ($question>=20), ($question>=11 AND $question<20),"
                    ...
                    ?><td class="all_projects"><?php
                            ...
                            /*        $check_order++;
                              ...
                              /*      if ($value == $check_order) {
                                   ...
                                   /*     goto backup;
                                    ...
                                    elseif ($question_count ==5 || $question_count==6){
                                        if ($key == 0) {
                                            $string.= '' . $value . ' days: ';
                                        ...
                                        /* questions 0 and 1 (B&H) are answered in number of days: */
                                        if ($key == 0) {
                                            $string.= '' . $value . ' days: ';
                                            $script_str.='["' . $value . ' days",';
                                        }
                               ...
                               /*end the not-check-numbers else }*/
                        ...
                        if ($i == 1) {
                            echo "Pre-survey average: " . number_format($pre[$question_count], 2);
                            $first_display = $perc_display;
                            echo "<br>";
                            $num_surveys = "SELECT COUNT(*) AS count FROM Parent_Mentor_Survey WHERE Pre_Post='$i' " . $year_post . $school_post . " AND $question IS NOT NULL";
                            $surveys=mysqli_query($cnnLSNA, $num_surveys);
                            $surv=mysqli_fetch_row($surveys);
                            echo "Number of surveys included: " . $surv[0];
                        }
                        if ($i == 2) {
                            echo "Mid-survey average: " . number_format($mid[$question_count], 2);
                        ...
                        if ($i == 3) {
                            echo "Post-survey average: " . number_format($post[$question_count], 2);
...
<?php //  echo $script_str; ?>
                    ...
                    <script type="text/javascript">
                        ...
                        $(document).ready(function() {
                            ...
                            var plot2 = $.jqplot('pm_survey_chart<?php $chart_number = $question_count + 1;
            echo $chart_number;
